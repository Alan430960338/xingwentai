	...
	<view class="page-wrap">
		...
		<view class="filter-wrap">
			...
			<view class="search-input">
				...
				<input class="search-placeholder" placeholder="搜索标题、故障内容、师傅" v-model="searchKeyword"
					@input="handleSearch" />
			</view>
			...
			<view class="order-card" v-for="order in orderList" :key="order.id" @click="gotoDetail(order)">
				...
				<view class="order-footer">
					...
					<view class="order-actions">
						...
						<view class="item-btn" v-if="[2, 3, 6].includes(order.status)"
							@click.stop="viewProgress(order)">
							查看进度
						</view>

						<view class="item-btn" v-if="order.status === 3 && order.is_wg==1"
							@click.stop="confirmCompletedWindow(order)">
							确认完成
						</view>
						<view class="item-btn" v-if=" order.service_name!==null"
							@click.stop="confirmCompletedWindow(order)">
							师傅资质
						</view>
						<view class="item-btn" v-if=" order.status === 5 && order.is_pj!==1"
							@click.stop="evaluateOrderWindow(order)">
							评价
						</view>
...
<script setup>
	...
	// ============ 方法 ============
	const evaluateOrderWindow = (order) => {
		uni.navigateTo({
			url: `/pages/user/order/evaluate?id=${order.id}`
		})
	}
		const statusMap = {
			0: '未支付',
		...
		const normalizeOrderList = (list = []) => {
			return list.map(item => ({
				...item,
				status: item.status === '' || item.status === null || item.status === undefined ? item.status :
					Number(item.status),
		...
		const getStatusColor = (status) => {
			...
			return colorMap[formatStatus(status)] || {
				bg: '#f5f5f5',
				color: '#999'
			}
		}
		...
		// 确认完成弹窗
		const confirmCompletedWindow = (order) => {
			uni.showModal({
				title: '确认完成',
				content: '确认已完成订单',
				success: (res) => {
					if(res.confirm)
					confirmCompleted(order)
				}
			})
		}
